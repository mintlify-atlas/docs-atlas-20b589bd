...
description: "Step-by-step guide to deploying the bot system on Oracle Cloud's Always Free Tier: 2 vCPU, 12GB RAM, 200GB storage — permanently free with no credit card charges."
---
...
<Tip>
  ...
  Then update `DASHBOARD_CALLBACK_URL` in `.env` to `https://your-domain.com/auth/discord/callback` and run `pm2 restart all`.
</Tip>
