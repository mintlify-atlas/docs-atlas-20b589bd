---
title: "Slash Command Reference for Capinetta RP Bots"
...
|---|---|---|---|
| `/ticket` | `add <nombre> <emoji> <rol> <categoria_discord> <descripcion>` | Create a new ticket category with a primary role, Discord category channel, emoji, and description. Optionally accepts two extra roles (`rol_extra_1`, `rol_extra_2`). | Administrator |
...
| `/reset-warns` | `@usuario` | Clear a user's warning counter in the database, resetting them to zero. | Administrator |
| `/history` | `@usuario` | Retrieve a user's full sanction history including warns, kicks, and bans with timestamps. | Manage Messages |
| `/unmute` | `@usuario` | Lift a user's sanction and restore all roles that were saved to the `warns.roles` JSON column before isolation. | Manage Roles |
...
| `/clear` | `amount` | Bulk delete up to 100 messages in the current channel using Discord's `bulkDelete` API. | Manage Messages |
...
| Command | Description | Permission |
|---|---|---|
| `/stats` | Displays a real-time technical dashboard embed with: CPU load average, RAM usage (system + Node.js heap + RSS), process uptime, disk info, Discord bot WebSocket latency, MariaDB connection latency, live ticket counts (open / unclaimed / closed / archived), warn log count, system error count, and guild member breakdown. Results are cached for 10 seconds to prevent spam. | Administrator |
| `/ping` | Quick latency check — shows the bot's WebSocket heartbeat ping to Discord and tests the MariaDB connection round-trip time. Available to all members. | Everyone |
