---
title: "Environment Variables Reference for Capinetta RP"
...
<ParamField path="ALERT_MEMORY_USAGE" type="number" default="85">
  Maximum system memory usage percentage. Alert fires when RAM usage exceeds this value. Default: `85` (alert at ≥85% memory usage). Note: `.env.example` comments show `80%` but the actual default in `config.js` is `85`.
</ParamField>
