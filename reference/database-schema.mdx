---
title: "Database Schema Reference: Prisma Models"
...
Records unhandled bot errors for post-mortem debugging. The `unhandledRejection` and `uncaughtException` handlers in both `index-general.js` and `index-whitelist.js` write here in production. Rows can be reviewed via `/db-tables` or Prisma Studio.
